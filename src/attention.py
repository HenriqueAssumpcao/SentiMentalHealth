...
class AttentionLayer(nn.Module):
    ...
    def calc_score(self, att_query, att_keys):
        ...
        if self.mode == 'bahdanau':
            att_query = att_query.unsqueeze(2).expand(b, t_q, t_k, n).contiguous()
            att_keys = att_keys.unsqueeze(1).expand(b, t_q, t_k, n).contiguous()
            sum_qk = att_query + att_keys
            sum_qk = sum_qk.view(b * t_k * t_q, n)
            out = self.linear_att(self.tanh(sum_qk)).view(b, t_q, t_k)
